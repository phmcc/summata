...
For continuous variables, `desctable()` computes location and scale parameters:

| Statistic | Formula | Use Case |
|:----------|:--------|:---------|
| Mean ± SD | $\bar{x} \pm s$ | Symmetric distributions |
| Median (IQR) | $\tilde{x}$ (Q1–Q3) | Skewed distributions |
| Median (Range) | $\tilde{x}$ (min–max) | Small samples, outlier detection |

For categorical variables, the function tabulates counts and proportions:

| Statistic | Notation | Description |
|:----------|:---------|:------------|
| n (%) | $n_i$ ($100 \cdot n_i / N$) | Count and percentage |
| n only | $n_i$ | Count without percentage |
| % only | $100 \cdot n_i / N$ | Percentage without count |

## Hypothesis Tests

When `test = TRUE`, the function selects appropriate tests based on variable type:

| Variable Type | Two Groups | Three or More Groups |
|:--------------|:-----------|:---------------------|
| Continuous (mean-based) | Student's *t*-test | One-way ANOVA |
| Continuous (median-based) | Wilcoxon rank-sum | Kruskal–Wallis |
| Categorical | χ² test | χ² test |
| Categorical (sparse cells) | Fisher's exact | Fisher's exact |
| Time-to-event | Log-rank | Log-rank |

For continuous variables with `test_continuous = "auto"`, the test selection follows the displayed statistic: parametric tests (*t*-test, ANOVA) are used when `stats_continuous` includes mean-based statistics, while nonparametric tests (Wilcoxon, Kruskal–Wallis) are used for median-based statistics. This ensures consistency between the summary measure and its associated test. For categorical variables, the function computes expected cell frequencies and uses Fisher's exact test when any expected count falls below 5.
...
Here `data` is the dataset, `by` specifies the grouping variable (optional), and `variables` lists the variables to summarize.
...
## **Example 6:** Automatic Test Selection

Setting `test = TRUE` enables automatic hypothesis testing based on the summary statistic being displayed:
...
Override automatic selection with `test_continuous` and `test_categorical`. The following example forces nonparametric tests for continuous variables:
...
example7a <- desctable(
  ...
  variables = c("age", "bmi", "los_days"),
  test = TRUE,
  test_continuous = "wrs",  # Wilcoxon rank-sum
  labels = clintrial_labels
)
...
Available test specifications include:
...
| Value | Effect |
|:------|:-------|
| `TRUE` | Total column first (default) |
| `"first"` | Total column first |
