...
For count outcomes, use `glmforest()` with a Poisson model:
...
poisson_model <- glm(
  event_count ~ age + sex + stage + treatment,
  data = clintrial,
  family = poisson
)
...
example22 <- glmforest(
  x = poisson_model,
  data = clintrial,
  title = "Poisson Regression: Event Counts",
  labels = clintrial_labels,
...
| `color` | Color for points and lines | `"#2171B5"` |
